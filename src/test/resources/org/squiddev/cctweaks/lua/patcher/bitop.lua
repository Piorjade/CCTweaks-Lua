...
local debug = assert.debug
local assertEq = assert.assertEquals
...
assertEq(0x7fffffff, 2147483647, "broken hex literals")
assert(0xffffffff == -1 or 0xffffffff == 2 ^ 32 - 1, "broken hex literals")
assertEq(tostring(-1), "-1", "broken tostring()")
assert(tostring(0xffffffff) == "-1" or tostring(0xffffffff) == "4294967295", "broken tostring()")

-- Basic argument processing.
assertEq(bit.tobit(1), 1, "tobit")
assertEq(bit.band(1), 1, "band")
assertEq(bit.bxor(1, 2), 3, "bxor")
assertEq(bit.bor(1, 2, 4, 8, 16, 32, 64, 128), 255, "bor")
